...
has_manager_role = rules.is_group_member(str(AUTH_ROLE.MANAGER))
...
rules.add_perm('engine.task.create', has_admin_role | has_manager_role | has_user_role)
rules.add_perm('engine.task.access', has_admin_role | has_manager_role | has_observer_role |
    is_task_owner | is_task_annotator)
rules.add_perm('engine.task.change', has_admin_role | has_manager_role | is_task_owner |
    is_task_assignee)
rules.add_perm('engine.task.delete', has_admin_role | has_manager_role | is_task_owner)

rules.add_perm('engine.job.access', has_admin_role | has_manager_role | has_observer_role |
    is_job_owner | is_job_annotator)
rules.add_perm('engine.job.change', has_admin_role | has_manager_role | is_job_owner |
    is_job_annotator)

rules.add_perm('engine.views.is_staff', has_admin_role | has_manager_role)

rules.add_perm('dashboard.views.isAdmin', has_admin_role)
rules.add_perm('dashboard.views.isManager', has_admin_role | has_manager_role)

rules.add_perm('dashboard.views.getAllUsersFromDB', has_admin_role | has_manager_role)
rules.add_perm('dashboard.views.getUsersForProjectFromDB', has_admin_role | has_manager_role)
rules.add_perm('dashboard.views.saveUsersForProjectToDB', has_admin_role | has_manager_role)
rules.add_perm('dashboard.views.getLabelsViewDb', has_admin_role)
rules.add_perm('dashboard.views.saveLabelAndAttrs', has_admin_role)
rules.add_perm('dashboard.views.saveFrameProperty', has_admin_role)
    ...
    constructor(drawerModel) {
        ...
        function setupShortkeys() {
            ...
            let switchDrawHandler = Logger.shortkeyLogDecorator(function() {
                // Enable drawing only if comment textarea is not open.
                if (!$("#commentTooltipTextArea").hasClass("commentAreaOpen")) {
    ...
    _create() {
        ...
        case 'box':
            this._drawInstance = this._frameContent.rect().draw({snapToGrid: 0.1}).addClass('shapeCreation').attr({
                ...
                this._controller.switchCreateMode(true);
                let currShapes = this._controller._model._listeners[1]._currentShapes;
                let model = currShapes[Object.keys(currShapes).reduce(function(a, b){return currShapes[a] > currShapes[b] ? a : b })].model

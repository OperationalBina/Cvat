...
class ShapeModel extends Listener {
    ...
    switchOutside(frame) {
        ...
        this.stopTrackingShape();

        let frameToDelete = frame + 1;
        while (this._positions[frameToDelete] !== undefined) {
            delete this._positions[frameToDelete];
            frameToDelete++;
        }
    ...
    startTrackingShape() {
        ...
        let startFrame = window.cvat.player.frames.current;
        $(this._listeners[1]._uis.buttons['play']).hide();
        $(this._listeners[1]._uis.buttons['stop']).hide();
        $(this._listeners[1]._uis.buttons['loadIcon']).show();
        if ($(".playButton:visible").length == 0) {
            $("#trackAllShapes").val("Stop").html(" Stop Tracking ");
        }
        let latestFrame = Object.keys(this._positions).map(x => +x).sort((a,b) => a - b).filter(x => x <= startFrame).slice(-1)[0];
        let dataToSend = {
            'positions': {
                'x': this._positions[latestFrame]["xtl"],
                'y': this._positions[latestFrame]["ytl"],
                'w': this._positions[latestFrame]["xbr"] - this._positions[latestFrame]["xtl"],
                'h': this._positions[latestFrame]["ybr"] - this._positions[latestFrame]["ytl"],
            },
            'id': this.id,
            'occluded': this._positions[latestFrame]["occluded"],
            'outside': this._positions[latestFrame]["outside"],
            'z_order': this._positions[latestFrame]["z_order"],
            'frame': startFrame,
            'frameHeight': window.cvat.player.geometry.frameHeight,
            'frameWidth': window.cvat.player.geometry.frameWidth
        };

        let trackedShape = this;
        $(trackedShape._listeners[1]._uis.buttons['progress'].children[0]).html(startFrame);
        $("#playerFrame").data("trackingStatus")[this.id] = {"play": false, "loading": true, "progress": "0%", "frame": startFrame};
        
        this._interval = setInterval(function() {
            $.ajax({
                type: 'POST',
                url: "track_full_video/" + window.cvat.job.taskId,
                data: JSON.stringify(dataToSend),
                contentType: "application/json; charset=utf-8",
                success: function(response) {
                    if (response !== "Downloading...") {
                        if ($("#playerFrame").data("trackingStatus")[trackedShape.id]["loading"]) {
                            $("#playerFrame").data("trackingStatus")[trackedShape.id]["loading"] = false;
                            $(trackedShape._listeners[1]._uis.buttons['stop']).show();
                            $(trackedShape._listeners[1]._uis.buttons['loadIcon']).hide();
                        }

                        Object.assign(trackedShape._positions, response["results"]);
                        let latestTrackedFrame = Object.keys(response["results"]).map(x => +x).sort((a,b) => a - b).slice(-1)[0];
                        $("#playerFrame").data("trackingStatus")[trackedShape.id]["frame"] = latestTrackedFrame;

                        $(trackedShape._listeners[1]._uis.buttons['progress'].children[0]).html(latestTrackedFrame);

                        let progress = ((latestTrackedFrame - startFrame) / (window.cvat.player.frames.stop - startFrame)) * 100
                        $("#playerFrame").data("trackingStatus")[trackedShape.id]["progress"] = progress.toString() + "%";
                        $(trackedShape._listeners[1]._uis.buttons['progress'].children[0]).css('width', progress.toString() + "%");

                        if (response['finished']) {
                            clearInterval(trackedShape._interval);
                            $(trackedShape._listeners[1]._uis.buttons['progress'].children[0]).css('width', '100%');
                            $(trackedShape._listeners[1]._uis.buttons['stop']).hide();
                            $(trackedShape._listeners[1]._uis.buttons['play']).show();
                            $("#playerFrame").data("trackingStatus")[trackedShape.id]["play"] = true;
                            $("#playerFrame").data("trackingStatus")[trackedShape.id]["progress"] = "100%";
                            if (($(".stopButton:visible").length == 0) && ($(".Ids-ring:visible").length == 0)) {
                                $("#trackAllShapes").val("Track").html(" Track All Shapes ");
                            }
                        }
                    }
                },
                error: (data) => {
                    showMessage(`Can not track video. Code: ${data.status}. Message: ${data.responeText || data.statusText}`);
                }
            });
        }, 1000);
    }
    ...
    stopTrackingShape() {
        ...
        $.ajax({
            type: 'GET',
            url: "stop_video_tracking/" + window.cvat.job.taskId + "/" + this.id,
            success: function(response) {
                if (response !== "No tracker stopped") {
                    if (trackedShape._listeners[1] !== undefined) {
                        $(trackedShape._listeners[1]._uis.buttons['stop']).hide();
                        $(trackedShape._listeners[1]._uis.buttons['play']).show();
                        $(trackedShape._listeners[1]._uis.buttons['progress'].children[0]).css('width', '100%');
            ...
            error: (data) => {
                showMessage(`Can not stop tracking video. Code: ${data.status}. Message: ${data.responeText || data.statusText}`);
            }
        });
    }
    ...
    updatePosition(frame, position, silent) {
        ...
        if (this._verifyArea(pos)) {
            ...
            if (this._type === 'annotation_box') {
                ...
                $("#playerFrame").data("predictions")[this._id] = {};
                startTracking(this);
            }
        }
...
class ShapeController {
    ...
    startTrackingShape() {
        this._model.startTrackingShape();
    }

    stopTrackingShape() {
        this._model.stopTrackingShape();
    }
...
class ShapeView extends Listener {
    ...
    _drawMenu(outside) {
        ...
        function makeButtonBlock(type, outside, shortkeys) {
            ...
            if (type.split('_')[0] == 'interpolation') {
                ...
                if (type.split('_')[1] == 'box') {
                    ...
                    if (!outside && window.cvat.job.video) {
                        var centerDiv = document.createElement('div');

                        centerDiv.classList.add('centerDiv');
                        let trackingStatus = $("#playerFrame").data("trackingStatus")[this._controller._model.id]
    
                        let playButton = document.createElement('button');
                        playButton.classList.add('graphicButton', 'playButton');
    
                        let stopButton = document.createElement('button');
                        stopButton.classList.add('graphicButton', 'stopButton');
                        ...
                        loadingIcon.appendChild(document.createElement("div"));
                        loadingIcon.appendChild(document.createElement("div"));
                        loadingIcon.classList.add('Ids-ring');
                        
                        this._uis.buttons['play'] = playButton;
                        this._uis.buttons['loadIcon'] = loadingIcon;
                        this._uis.buttons['stop'] = stopButton;
                        
                        centerDiv.appendChild(playButton);
                        centerDiv.appendChild(loadingIcon);
                        centerDiv.appendChild(stopButton);
                        ...
                        if (trackingStatus !== undefined) {
                            ...
                            barDiv.innerHTML = trackingStatus["frame"];
                            barDiv.style.width = trackingStatus["progress"];
                        } else {
                            stopButton.style.display = "none";
                            loadingIcon.style.display = "none";
                        }
                        
                        this._uis.buttons['progress'] = progressDiv;
                ...
                if (typeof centerDiv !== "undefined") buttonBlock.appendChild(centerDiv);
        ...
        function makeChangeLabelBlock(shortkeys) {
            ...
            if (Object.keys(labels).length > 1) {
                ...
                // Upon label selected, update its color in the UI
                $(select).on("change", {'boxView': this}, function(e) {
                    let boxView = e.data.boxView; 
                    let color = cvat.labelsInfo._labels[this.value].color; 
                    
                    var colorSettings = {
                        'color-by-label': true,
                        'color-by-group': false,
                        'colors-by-label':  (() => {return color}), // Function which always return the new label's color
                        'fill-opacity': 0
                    }

                    // Update the color in the view & model.
                    boxView._appearance.colors = {'shape': color, 'ui': color}
                    boxView._controller._model._color = boxView._appearance.colors;
                    boxView.updateColorSettings(colorSettings);
                })
    ...
    _activateMenu() {
        ...
        if ('play' in this._uis.buttons) {
            this._uis.buttons.play.onclick = () => {
                this._controller.startTrackingShape();
                _paq.push(['trackEvent', 'Tracker', 'Track box', cvat.project.name])
            };
        }

        if ('stop' in this._uis.buttons) {
            this._uis.buttons.stop.onclick = () => {
                this._controller.stopTrackingShape();
                _paq.push(['trackEvent', 'Tracker', 'Stop track box', cvat.project.name])
            };
        }
    ...
    _applyColorSettings() {
        ...
        if (this._uis.menu) {
            let color = this._appearance.fill ? this._appearance.fill : this._appearance.colors.ui;
            
            this._uis.menu.css({
                'background-color': color,
            });

            for (let uiButton of Object.values(this._uis.buttons)) {
                if (isDark(color)) {
                    uiButton.classList.add('invertColor');
                    this._uis.menu.find('label').addClass('invertColor');
                } else {
                    uiButton.classList.remove('invertColor');
                    this._uis.menu.find('label').removeClass('invertColor');
                }
            }    
        }

        function isDark(colorHex) {
            let c = colorHex.substring(1); // strip #
    ...
    updateColorSettings(settings) {
        if ('white-opacity' in settings) {
            ...
            if (settings['color-by-group']) {
                ...
                let color = window.cvat.labelsInfo.labelColor(this._controller.label);
                this._appearance.stroke = color;
                this._appearance.fill = color;
            }
        }

...
class ShapeCollectionModel extends Listener {
    ...
    add(data, type, currentFrame = 0) {
        if (type.startsWith('previousFrame')) {
            let result = this._generateCorrectData(data, type, currentFrame);
            data = result[0];
            type = result[1];
        }
        let id = 'id' in data && data.id !== -1 ? data.id : this._idGen.next();

        let model = buildShapeModel(data, type, id, this.nextColorForInstance());
    ...
    _generateCorrectData(data, type, currentFrame) {
        type = data.type;
        let group_id = data.groupId;
        let label_id = data.label;
        let z_order = data._positions[data.frame].z_order;
        let points;
        let xbr;
        let xtl;
        let ybr;
        ...
        if (type != "annotation_box") {
            points = data._positions[data.frame].points;
        } else {
            xbr = data._positions[data.frame].xbr;
            xtl = data._positions[data.frame].xtl;
            ybr = data._positions[data.frame].ybr;
            ytl = data._positions[data.frame].ytl;
        }
        data = {
            "attributes": [],
            "frame": currentFrame,
    ...
    onPlayerUpdate(player) {
        if (player.ready()) {
            ...
            if (this._loadedFrames[frame] !== undefined && this._loadedFrames[frame] !== '') {
                $('#loadedFrames').html(this._loadedFrames[frame]);
            } else {
                $('#loadedFrames').html(" - ");
            }
        }
    ...
    switchActiveColor() {
        ...
        if (activeShape) {
            if (colorByInstanceInput.prop('checked')) {
                activeShape.changeColor(this._model.nextColorForInstance());
            }
...
class ShapeCollectionView {
    constructor(collectionModel, collectionController) {
        ...
        this._framePropsContent = $('#framePropsContent');
        this._segmentContent = $('#segmentContent');
        this._trackAllShapesButton = $('#trackAllShapes');
        this._startTrackingButton = $('#startTrackingButton');
        this._createEventButton = $('#createEventButton');
        this._eventTypeSelector = $('#eventTypeSelector');
        ...
        this._fillWatershedOpacityRange = $('#fillWatershedOpacityRange');
        ...
        let labelsSegmentationInfo = window.cvat.labelsInfo.segmentation();
        for (let labelName in labelsSegmentationInfo) {
            let labelInfo = labelsSegmentationInfo[labelName];
            let option = $(`<tr><td><div class="color-picker regular h2" style="background-color: ${labelInfo['color']};" data-color=${labelInfo['color']} data-typeid=${labelInfo['typeId']} data-labelid=${labelInfo['id']}>${labelName}</div></td></tr>`);
            option.appendTo(this._segmentContent.find('table'));
        }
        ...
        this._fillWatershedOpacityRange.on('input', (e) => {
            let value = Math.clamp(+e.target.value, +e.target.min, +e.target.max);
            e.target.value = value;

            // update frame watershed opcaity
            $("#frameWatershed").css("opacity", value)
        });
        ...
        let frameProperties = window.cvat.frameProperties;
        var colorIndexer = 1

        for (var propertyName in frameProperties) {

            // Create the button opening the values
            let downButton = $(`<button> </button>`)
                .addClass('graphicButton downArrowButton')
                .attr('title', 'Choose the values for this frame property.')
                .on('click', (event) => {
                    
                    // Get the form of the property clicked
                    var propertyForm = event.target.parentNode.parentNode.children[1]

                    // Show/hide the values of the current property
                    if (propertyForm.hasAttribute("hidden")) {
                        propertyForm.removeAttribute("hidden")
                        $(event.target).removeClass("downArrowButton").addClass("upArrowButton")
                    } else {
                        propertyForm.setAttribute("hidden", true)
                        $(event.target).removeClass("upArrowButton").addClass("downArrowButton")
                    }   
                });

            // Create the div inside the panel
            let buttonBlock = $('<center> </center>').append(downButton).addClass('buttonBlockOfFramePropUI');
            let title = $(`<label style="display: flex"> ${propertyName} </label>`);
            let clearSelection = $(`<a class="clearSelectionText">(clear selection)</a>`)
            clearSelection.on("click", {propertyName: propertyName}, (event) => {
                // Get the current form and uncheck the correct radio button
                var currForm = event.target.parentNode.parentNode.children[1]
                $('input:radio:checked', currForm).prop("checked", false)
                clearFramePropJSON(event);
            })
            title.append(clearSelection);

            // Create the radio buttons menu for the values
            let propertyValues = $(`<form id="${propertyName}" style="margin: 10px 0px 10px 0px" hidden>`)
            for (var valueName in frameProperties[propertyName]) {
                let radioButton = $(`<input type="radio" id="${valueName}" name="${propertyName}" value="${frameProperties[propertyName][valueName]}" />`)

                // Update the JSON whenever a new value is being selected
                radioButton.on("change", {id: frameProperties[propertyName][valueName], propertyName: propertyName, valueName: valueName}, updateFramePropJSON)

                let radioContainer = $(`<div style="display: -webkit-box;">`).append(radioButton)
                                        .append($(`<label for="${valueName}" class='framePropValueText'>${valueName}</p></div>`))       
                propertyValues.append(radioContainer)
            }

            propertyValues.append($(`</form>`))

            // Create the div containing the whole menu
            let mainDiv = $('<div> </div>').addClass('framePropContentElement h2 regular')
                .css({
                    // Get a random color from the labels menu
                    'background-color': collectionController.colorsByGroup(+window.cvat.labelsInfo.labelColorIdx(colorIndexer)),
                }).on('mouseover', () => {
                    mainDiv.addClass('highlightedUI');
                }).on('mouseout', () => {
                    mainDiv.removeClass('highlightedUI');
                }).append(title).append(propertyValues).append(buttonBlock);

            this._framePropsContent.append(mainDiv);

            colorIndexer++;
        }

        function clearFramePropJSON(event) {
            var propertyName = event.data["propertyName"]
            for (var i = cvat.player.frames.current; i <= (cvat.player.frames.stop) && (!cvat.framePropertiesJSON[i][propertyName].isStartingFrame) ; i++) {
                    cvat.framePropertiesJSON[i][propertyName] = {};
            }
        }

        function updateFramePropJSON(event) {
            var propertyName = event.data["propertyName"]
            var propertyValue = event.data["propertyValue"]

            cvat.framePropertiesJSON[cvat.player.frames.current][propertyName] = { "propValId": event.data.id, "isStartingFrame": true }

            // Change the JSON until the next starting frame key is hit for the current property OR all the frames were checked.
            for (var i = cvat.player.frames.current+1; (i <= cvat.player.frames.stop) && (!cvat.framePropertiesJSON[i][propertyName].isStartingFrame); i++) {
                cvat.framePropertiesJSON[i][propertyName] = { "propValId": event.data.id, "isStartingFrame": false }
            }
        }

        let sidePanelObjectsButton = $('#sidePanelObjectsButton');
        let sidePanelLabelsButton = $('#sidePanelLabelsButton');
        let sidePanelSegmentationButton = $('#sidePanelSegmentationButton');

        sidePanelObjectsButton.on('click', () => {
            if (!sidePanelObjectsButton.hasClass('activeTabButton')) {
                sidePanelObjectsButton.addClass('activeTabButton');
                sidePanelLabelsButton.removeClass('activeTabButton');
                sidePanelSegmentationButton.removeClass('activeTabButton');
                this._UIContent.removeClass('hidden');
                this._framePropsContent.addClass('hidden');
                this._segmentContent.addClass('hidden');

                this._trackAllShapesButton.removeClass('hidden');
                this._startTrackingButton.removeClass('hidden');

                if ($('#showCanvasCheckbox').prop('checked')) {
                    $('#showCanvasCheckbox').trigger('click');
                }
                ...
                $('#trackManagement').removeClass('hidden');
                $('#playerBottom').children().removeClass('hidden');
                $('#commentDiv').removeClass('nudge-right');
                $('#segmentManagement').addClass('hidden');
                $('#SegmentationBar').addClass('hidden');

                this._frameWatershed.addClass('hidden');

                $.ajax({
                    url: 'segmentation/exit/task/' + window.cvat.job.taskId,
                    type: 'PUT',
                    async: false
                });
            }
        ...
        sidePanelSegmentationButton.on('click', () => {
            if (!sidePanelSegmentationButton.hasClass('activeTabButton')) {
                sidePanelSegmentationButton.addClass('activeTabButton');
                sidePanelObjectsButton.removeClass('activeTabButton');
                sidePanelLabelsButton.removeClass('activeTabButton');
    
                // Reseting the size of image and activating onPlayerUpdate
                $('#playerFrame').trigger('segmentationmode');
    
                this._segmentContent.removeClass('hidden');
                this._framePropsContent.addClass('hidden');
                this._UIContent.addClass('hidden');

                this._trackAllShapesButton.addClass('hidden');
                this._startTrackingButton.addClass('hidden');
    
                $('#showCanvasCheckbox').trigger('click');
                $('#showWatershedCheckbox').trigger('click');
    
                $('#frameContent').addClass('hidden');
                $('#trackManagement').addClass('hidden');
                $('#playerBottom').children().addClass('hidden');
                $('#commentDiv').addClass('nudge-right');
                $('#rightPanelDiv').removeClass('hidden');
                $('#menuButton').removeClass('hidden');
                $('#segmentManagement').removeClass('hidden');
                $('#SegmentationBar').removeClass('hidden');
                $('#SegmentationBar').removeClass('hidden');
                this._frameWatershed.removeClass('hidden');
        ...
        this._colorByLabelRadio.trigger("change");
    }
    ...
    onCollectionUpdate(collection) {
        ...
        if (frameChanged) {
            parents.shapes.appendChild(this._frameContent.node);
        ...
        if (this._controller._model._loadedFrames[this._frameMarker] !== undefined &&
            this._controller._model._loadedFrames[this._frameMarker] !== '') {
            $('#loadedFrames').html(this._controller._model._loadedFrames[this._frameMarker]);
        } else {
            $('#loadedFrames').html(" - ");
        }
    ...
    onPlayerUpdate(player) {
        if (!player.ready())  this._frameContent.addClass('hidden');
            ...
            if (!$('#sidePanelSegmentationButton').hasClass('activeTabButton')) {
                this._frameContent.removeClass('hidden');
            }
        }

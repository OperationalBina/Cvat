...
@silk_profile(name="Save keyframe_spec")
@transaction.atomic
def save_keyframe(taskid, frameProperties):
    # db_taskframespec = models.TaskFrameSpec.objects.filter(task_id=taskid)
    # for currTaskFrameSpec in db_taskframespec:
    models.KeyframeSpec.objects.filter(frameSpec__task_id=taskid).delete()

    for frame in frameProperties.keys():
        for propValId in frameProperties[frame]:
            db_taskFrameSpec = models.TaskFrameSpec.objects.get(task_id=taskid, propVal_id=propValId)
            try:
                latestPk = (models.KeyframeSpec.objects.latest('pk').pk) + 1
            except:
                latestPk = 0

            models.KeyframeSpec.objects.create(pk=latestPk, frame=frame, frameSpec=db_taskFrameSpec)

    

@silk_profile(name="Save job")
@transaction.atomic
def save_job(jid, data, taskid=0, frameProperties=None):
    """
    ...
    if frameProperties is not None:
        save_keyframe(taskid, frameProperties)
        ...
        with open(dump_path, "w") as dump_file:
            ...
            if db_task.mode == "annotation":
                ...
                for frame in sorted(set(list(shapes["boxes"].keys()) +
                    ...
                    frame_file = link.split(os.path.sep)[-1]
